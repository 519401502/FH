                                    ...
                                    <c:forEach items="${varList}" var="var" varStatus="vs">
                                        <tr>
                                            <td class='center' style="width: 30px;">
                                                <label><input type='checkbox' name='ids' value="${var.ORDER_ID}"/><span
                                                        class="lbl"></span></label>
                                            </td>
                                            <td class='center' style="width: 30px;">${vs.index+1}</td>
                                            <td>${var.ACCOUNT_ID}</td>
                                            <c:if test="${var.PAY_TYPE != null && var.PAY_TYPE == 'ali'}">
                                                <td>支付宝</td>
                                            </c:if>
                                            <c:if test="${var.PAY_TYPE != null && var.PAY_TYPE == 'wx'}">
                                                <td>微信支付</td>
                                            </c:if>
                                            ...
                                            <td>
                                            <%--${var.PAY_STATUS}--%>
                                                <c:if test="${var.PAY_STATUS != null && (var.PAY_STATUS == 'n' || var.PAY_STATUS == '未支付')}">
                                                    未支付
                                                </c:if>
                                                <c:if test="${var.PAY_STATUS != null && (var.PAY_STATUS == 'y' || var.PAY_STATUS == '已支付')}">
                                                    已支付
                                                </c:if>
                                            </td>
                                            ...
                                            <%--<td>${var.EXPRESS_NO}</td>--%>
                                            <%--<td>${var.EXPRESS_COMPANY_NAME}</td>--%>
                                            <td style="text-align: center">
                                                <c:if test="${var.PAY_STATUS != null && (var.PAY_STATUS == '已支付' || var.PAY_STATUS == 'y')}">
                                                    <c:if test="${var.STATUS != null && (var.STATUS == '已收单' || var.STATUS == '1')}">
                                                        <a onclick="alertState('${var.ORDER_ID}', '${var.STATUS}')"
                                                           target="_blank">发货</a>
                                                    </c:if>
                                                    <c:if test="${var.STATUS != null && (var.STATUS == '已发货' || var.STATUS == '2')}">
                                                        <a onclick="alertState('${var.ORDER_ID}', '${var.STATUS}')"
                                                           target="_blank">签收</a>
                                                    </c:if>
                                                    <c:if test="${var.STATUS != null && (var.STATUS == '已签收' || var.STATUS == '3')}">
                                                        <a onclick="alertState('${var.ORDER_ID}', '${var.STATUS}')"
                                                           target="_blank">归档</a>
                                                    </c:if>
                                                    <c:if test="${var.STATUS != null && (var.STATUS == '已归档' || var.STATUS == '4')}">
                                                           归档
                                                    </c:if>
                                                </c:if>
                                                <c:if test="${var.PAY_STATUS != null && (var.PAY_STATUS == '未支付' || var.PAY_STATUS == 'n')}">
                                                    订单还没支付呢,亲~
                                                </c:if>
                                            </td>
...
<script type="text/javascript">
    ...
    function del(Id) {
        bootbox.confirm("确定要删除吗?", function (result) {
            if (result) {
                top.jzts();
                var url = "<%=basePath%>order/delete.do?ORDERCONTROLLER_ID=" + Id + "&tm=" + new Date().getTime();
    ...
    function edit(Id) {
        ...
        diag.Drag = true;
        diag.Title = "编辑";
        diag.URL = '<%=basePath%>order/goEdit.do?ORDERCONTROLLER_ID=' + Id;
        diag.Width = 450;
        diag.Height = 355;
        diag.CancelEvent = function () { //关闭事件
            if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
