...
public class ArticleController extends BaseController {
	...
	enum ArticleType{
		//将 gank.io/api中的类型作出调整
		Android, IOS, 前端, WEB后台, 扩展资源, App杂谈
	}
	...
	public ModelAndView list(Page page){
		...
		try{
			...
			mv.setViewName("article/article/article_list");
	...
	public ModelAndView goAdd(){
		...
		pd = this.getPageData();
		try {
			mv.setViewName("article/article/article_edit");
	...
	public ModelAndView exportExcel(){
		...
		try{
			...
			for(int i=0;i<varOList.size();i++){
				...
				vpd.put("var2", varOList.get(i).getString("WHO"));	//2
